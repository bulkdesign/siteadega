...
function lfb_editItem(itemID) {
    ...
    if (itemID > 0) {
        ...
        jQuery('#lfb_winItem').find('[name="isHidden"]').prop('checked', false);
        jQuery('#lfb_winItem').find('[name="isRequired"]').prop('checked', false);
...
function lfb_addShowLayerInteraction(data) {
    ...
    jQuery.each(lfb_currentForm.fields, function () {
        ...
        if (item.type != 'richtext' && item.type != 'colorpicker' && item.type != 'shortcode' && item.type != 'separator' && item.type != 'layeredImage') {
            ...
            $select.append('<option value="' + itemID + '" data-type="' + item.type + '">' + finalStepTxt + ' : " ' + item.title + ' "</option>');
        }
    });
...
function lfb_linksUpdateFields($operatorSelect, data) {
    ...
    if ($operatorSelect.closest('.lfb_conditionItem').find('.lfb_conditionoperatorSelect option:selected').attr('data-variable') == "timefield") {
        if ($operatorSelect.closest('.lfb_conditionItem').find('.lfb_conditionValue').length == 0) {
            ...
            $operatorSelect.closest('.lfb_conditionItem').find('.lfb_conditionValue').timepicker({
                showMeridian: jQuery('#lfb_formFields [name="timeModeAM"]').is(':checked'),
                appendWidgetTo: '#estimation_popup.wpe_bootstraped'
            });
    ...
    setTimeout(function () {
        if (data && data.value) {
            if (data.value.indexOf('_') > -1) {
                //var stepID = data.value.substr(0,data.value.indexOf('_'));
        ...
        jQuery.each(lfb_currentForm.steps, function () {
            jQuery.each(this.items, function () {
                if (this.id == selectedItemID) {
                    ...
                    if (this.type == 'numberfield' || this.type == 'textfield' || this.type == 'textarea' || this.type == 'select' || this.type == 'colorpicker' || this.type == 'datepicker' || this.type == 'timepicker') {
                        $selectElement.find('option[value="value"]').show();
                        $selectElement.find('option[value=""]').hide();
                        $selectElement.val('value');
                    } else {
...
function lfb_updateEmailValueItems() {
    ...
    jQuery.each(lfb_currentForm.fields, function () {
        ...
        if (this.type == 'numberfield' || this.type == 'textfield' || this.type == 'textarea' || this.type == 'select' || this.type == 'colorpicker' || this.type == 'datepicker' || this.type == 'timepicker') {
            var itemID = item.id;
            $selectItem.append('<option value="' + itemID + '" data-type="' + item.type + '">' + lfb_data.texts['lastStep'] + ' : " ' + item.title + ' "</option>');
        }
        // $selectItem.append('<option value="f' + this.id + '" data-type="textfield">' + lfb_data.texts['lastStep'] + ' : " ' + this.title + ' "</option>');
    });
}
