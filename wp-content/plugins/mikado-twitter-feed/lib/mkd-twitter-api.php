...
include_once 'mkd-twitter-helper.php';

/**
 * Class MikadoTwitterApi
 */
class MikadoTwitterApi {
	/**
	...
	const REQUEST_TOKEN_FIELD = 'mkd_twitter_request_token';
	const REQUEST_TOKEN_SECRET_FIELD = 'mkd_twitter_request_token_secret';
	const ACCESS_TOKEN_FIELD = 'mkd_twitter_access_token';
	const ACCESS_TOKEN_SECRET_FIELD = 'mkd_twitter_access_token_secret';
	const AUTHORIZE_TOKEN_FIELD = 'mkd_twitter_authorize_token';
	const AUTHORIZE_VERIFIER_FIELD = 'mkd_twitter_authorize_verifier';
	const USER_ID_FIELD = 'mkd_twitter_user_id';
	const USER_SCREEN_NAME_FIELD = 'mkd_twitter_screen_name';
	...
	public function __construct() {
		...
		$this->helper          = new MikadoTwitterHelper();
	}
	 ...
	 *
	 * @see MikadoTwitterApi::generateSignatureBase()
	 */
	...
	public function obtainRequestToken() {
		...
		if(is_wp_error($response)) {
			...
			$responseObj->message = __('Internal WP error', 'fleur');
		} else {
				...
				if((is_array($responseParsed) && count($responseParsed)) &&
				   !empty($responseParsed['oauth_token']) && !empty($responseParsed['oauth_token_secret'])
					...
					if(!empty($responseObj->redirectUrl)) {
						$responseObj->status  = false;
						$responseObj->message = __('Redirect URL couldn\t not be generated', 'fleur');
					} else {
					...
					$responseObj->status  = false;
					$responseObj->message = __('Couldn\'t connect with Twitter API', 'fleur');
				}
			}
		}
			...
			if(is_wp_error($response)) {
				...
				if(is_array($responseParsed) && count($responseParsed)
				   && !empty($responseParsed['oauth_token']) && !empty($responseParsed['oauth_token_secret']) && !empty($responseParsed['user_id']) && !empty($responseParsed['screen_name'])
					...
					$responseObj->status  = true;
					$responseObj->message = __('Access token obtained', 'fleur');
				}
			}
		} else {
			$responseObj->status  = false;
			$responseObj->message = __('Authorize token and it\'s secret were not obtainer', 'fleur');
		}
	...
	public function fetchTweets($userId = '', $count = '', $transient = array()) {
		...
		if(!$this->transientExists($transient)) {
			if($userId && $accessToken) {
				...
				if(is_wp_error($response)) {
					...
					if(isset($response['response']['code']) && $response['response']['code'] == 200) {
						...
						$responseObj->status  = false;
						$responseObj->message = __('Couldn\'t connect with Twitter', 'fleur');
					}
				}
			} else {
				$responseObj->status  = false;
				$responseObj->message = __('It seams like you haven\t connected with your Twitter account', 'fleur');
			}
		} else {
			...
			if(!empty($transientContent)) {
				...
				$responseObj->status  = false;
				$responseObj->message = __('Couldn\'t retreive content from database', 'fleur');
			}
		}
