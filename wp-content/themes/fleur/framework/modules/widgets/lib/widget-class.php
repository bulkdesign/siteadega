 ...
 *
 * Class FleurMikadoWidget
 */
abstract class FleurMikadoWidget extends WP_Widget {
	/**
	...
	public function form($instance) {
		if(is_array($this->params) && count($this->params)) {
				...
				switch($param['type']) {
                    ...
                    case 'textarea': ?>
                        <p>
                            <label for="<?php echo esc_attr($this->get_field_id($param['name'])); ?>"><?php echo
                                esc_html($param['title']); ?>:</label>
                            <textarea rows="8" class="widefat" id="<?php echo esc_attr($this->get_field_id($param['name'])); ?>" name="<?php echo esc_attr($this->get_field_name($param['name'])); ?>"><?php print ${$param['name']}; ?></textarea>
                            <?php if(!empty($param['description'])) : ?>
                                <span class="mkd-field-description"><?php echo esc_html($param['description']); ?></span>
                            <?php endif; ?>
                        </p>
			...
			<p><?php esc_html_e('There are no options for this widget.', 'fleur'); ?></p>
		<?php }
	}
	...
	public function update($new_instance, $old_instance) {
		...
		foreach($this->params as $param) {
            ...
            if(current_user_can('unfiltered_html')) {
                ...
                $instance[$param_name] = stripslashes(wp_filter_post_kses(addslashes($new_instance[$param_name])));
            }
		}
