<?php
namespace Fleur\Modules\Shortcodes\Icon;

use Fleur\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package Fleur\Modules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface {
            ...
            'params'                    => array_merge(
                ...
                array(
                    ...
                    array(
                        'type'        => 'textfield',
                        'admin_label' => true,
                        'heading'     => esc_html__('Custom Size (px)', 'fleur'),
                    ...
                    array(
                        'type'        => 'textfield',
                        'heading'     => esc_html__('Border radius', 'fleur'),
                        'param_name'  => 'border_radius',
                        'description' => esc_html__('Please insert border radius(Rounded corners) in px. For example: 4 ', 'fleur'),
                    ...
                    array(
                        ...
                        'admin_label' => true,
                        'description' => esc_html__('Enter just number. Omit pixels', 'fleur'),
                        'dependency'  => array('element' => 'type', 'value' => array('circle', 'square'))
                    ),
                    array(
                        'type'        => 'colorpicker',
                    ...
                    array(
                        'type'        => 'colorpicker',
                        'heading'     => esc_html__('Hover Icon Color', 'fleur'),
                    ...
                    array(
                        'type'        => 'colorpicker',
                        'heading'     => esc_html__('Hover Border Color', 'fleur'),
                    ...
                    array(
                        'type'        => 'colorpicker',
                        'heading'     => esc_html__('Hover Background Color', 'fleur'),
                    ...
                    array(
                        'type'        => 'textfield',
                        'heading'     => esc_html__('Margin', 'fleur'),
                        'param_name'  => 'margin',
                        'admin_label' => true,
                        'description' => esc_html__('Margin (top right bottom left)', 'fleur')
                    ),
                    array(
                        'type'        => 'dropdown',
                    ...
                    array(
                        'type'        => 'checkbox',
                        'heading'     => esc_html__('Use Link as Anchor', 'fleur'),
                        'value'       => array('Use this icon as Anchor?' => 'yes'),
                        'param_name'  => 'anchor_icon',
                        'admin_label' => true,
                        'description' => esc_html__('Check this box to use icon as anchor link (eg. #contact)', 'fleur'),
    ...
    public function render($atts, $content = null) {
        ...
        $html = fleur_mikado_get_shortcode_module_template_part('templates/icon', 'icon', '', $params);
        ...
        if(($params['type'] !== 'normal' && !empty($params['shape_size'])) ||
           ($params['type'] == 'normal')
            ...
            if(!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:'.fleur_mikado_filter_px($params['custom_size']).'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if(isset($params['type']) && $params['type'] !== 'normal') {
            ...
            if(!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: '.fleur_mikado_filter_px($shapeSize + 1).'px'; //because some icones are cutted
                $iconHolderStyles[] = 'height: '.fleur_mikado_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'line-height: '.fleur_mikado_filter_px($shapeSize).'px';
            }
            ...
            if(!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
                ...
                $iconHolderStyles[] = 'border-width: '.fleur_mikado_filter_px($params['border_width']).'px';
            ...
            if($params['type'] == 'square') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: '.fleur_mikado_filter_px($params['border_radius']).'px';
                }
            }
        }
