...
class LFB_Core {
    ...
    public function __construct($file = '', $version = '1.6.0') {
        ...
        // add_filter('woocommerce_checkout_cart_item_quantity',array($this, 'customDataToWooOrder'),1,3);
        //add_filter('woocommerce_cart_item_price',array($this, 'customDataToWooOrder'),1,3);
    ...
    public function conditionally_add_scripts_and_styles($posts) {
        ...
        if (!$this->checkedSc) {
            ...
            if ($shortcode_found && count($this->currentForms) > 0) {
                ...
                wp_register_style($this->_token . '-bootstrap', esc_url($this->assets_url) . 'css/bootstrap.min.css', array(), $this->_version);
                wp_register_style($this->_token . '-bootstrap-select', esc_url($this->assets_url) . 'css/bootstrap-select.min.css', array(), $this->_version);
                wp_register_style($this->_token . '-bootstrap-timepicker', esc_url($this->assets_url) . 'css/timepicker.min.css', array(), $this->_version);
                wp_register_style($this->_token . '-flat-ui', esc_url($this->assets_url) . 'css/flat-ui_frontend.min.css', array(), $this->_version);
                wp_register_style($this->_token . '-colpick', esc_url($this->assets_url) . 'css/colpick.min.css', array(), $this->_version);
                wp_register_style($this->_token . '-dropzone', esc_url($this->assets_url) . 'css/dropzone.min.css', array(), $this->_version);
                wp_register_style($this->_token . '-fontawesome', esc_url($this->assets_url) . 'css/font-awesome.min.css', array(), $this->_version);
                ...
                wp_enqueue_style($this->_token . '-bootstrap');
                wp_enqueue_style($this->_token . '-bootstrap-select');
                //  wp_enqueue_style($this->_token . '-bootstrap-timepicker');
                wp_enqueue_style($this->_token . '-flat-ui');
                wp_enqueue_style($this->_token . '-colpick');
                wp_enqueue_style($this->_token . '-dropzone');
                wp_enqueue_style($this->_token . '-fontawesome');
                ...
                wp_register_script($this->_token . '-bootstrap-timepicker', esc_url($this->assets_url) . 'js/bootstrap-timepicker.min.js', array($this->_token . '-bootstrap'), $this->_version);
                wp_enqueue_script($this->_token . '-bootstrap-timepicker');
        ...
        if ($chkDisplay) {
            ...
            if ($dataItem->type == 'picture') {
                ...
                $response .= '<label>' . $dataItem->title . '</label>
                        ' . $conditionalWrapStart . '<input type="text"  placeholder="' . $dataItem->placeholder . '" data-itemid="' . $dataItem->id . '"  ' . $showInSummary . '  ' . $hideQtSummary . '   ' . $useShowConditions . ' ' . $showConditions . ' ' . $showConditionsOperator . '  class="form-control lfb_datepicker" ' . $itemRequired . ' data-title="' . $dataItem->title . '" data-originaltitle="' . $originalTitle . '"  ' . $urlTag .
                        ' data-allowpast="' . $dataItem->date_allowPast . '" data-showmonths="' . $dataItem->date_showMonths . '" data-showyears="' . $dataItem->date_showYears . '" />' . $conditionalWrapEnd . '
                                              ';
                ...
                if ($dataItem->description != "") {
                    $response .= '<p class="itemDes" style="margin: 0 auto; max-width: 90%; margin-top: 6px !important;">' . $dataItem->description . '</p>';
                }
                $response .= '</div>';
                ...
                $response .= '<div class="lfb_richtext lfb_item" data-title="' . $dataItem->title . '"  ' . $useShowConditions . ' ' . $showConditions . ' ' . $showConditionsOperator . '>' . do_shortcode($dataItem->richtext) . '</div>';
            } else if ($dataItem->type == 'shortcode') {
                $response .= '<div class="lfb_richtext lfb_item" data-title="' . $dataItem->title . '"  ' . $useShowConditions . ' ' . $showConditions . ' ' . $showConditionsOperator . '>' . do_shortcode($dataItem->shortcode) . '</div>';
              ...
              //  }
                ...
                $response .= '<label>' . $dataItem->title . '</label>
                     ' . $conditionalWrapStart . '<input type="number" placeholder="' . $dataItem->placeholder . '" ' . $useShowConditions . ' ' . $showConditions . '  ' . $hideQtSummary . '  ' . $showConditionsOperator . ' data-itemid="' . $dataItem->id . '" ' . $minLength . ' ' . $maxLength . ' ' . $showInSummary . ' ' . $urlTag . ' ' . $defaultValue . ' class="form-control" ' . $itemRequired . ' data-title="' . $dataItem->title . '" data-originaltitle="' . $originalTitle . '" />' . $conditionalWraEnd . '
                  ';
