 ...
 */
class FleurMikado_Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu {
	/**
	...
	function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
		...
		if(!empty($item->_invalid)) {
			...
			$title = sprintf(esc_html__('%s (Invalid)', 'fleur'), $item->title);
		} elseif(isset($item->post_status) && 'draft' == $item->post_status) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf(esc_html__('%s (Pending)', 'fleur'), $item->title);
		}
			...
			<dt class="menu-item-handle">
                ...
                <span class="item-controls">
                    ...
                    <span class="item-order hide-if-js">
                        ...
                        ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'fleur'); ?>">&#8593;</abbr></a>
                        |
                        <a href="<?php
                        echo esc_url(wp_nonce_url(
	                        add_query_arg(
                        ...
                        ?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down', 'fleur'); ?>">&#8595;</abbr></a>
                    </span>
                    <a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item', 'fleur'); ?>" href="<?php
                    ...
                    ?>"><?php esc_html_e('Edit Menu Item', 'fleur'); ?></a>
                </span>
			</dt>
		</dl>
		...
		<div class="menu-item-settings wp-clearfix" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
			...
			<p class="description description-thin">
				<label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Navigation Label', 'fleur'); ?><br/>
			...
			<p class="description description-thin">
				<label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Title Attribute', 'fleur'); ?><br/>
			...
			<p class="field-link-target description">
				<label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
					...
					<?php esc_html_e('Open link in a new window/tab', 'fleur'); ?>
				</label>
			</p>

			<p class="field-css-classes description description-thin">
				<label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('CSS Classes (optional)', 'fleur'); ?><br/>
			...
			<p class="field-xfn description description-thin">
				<label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Link Relationship (XFN)', 'fleur'); ?><br/>
			...
			<p class="field-description description description-wide">
				<label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Description', 'fleur'); ?><br/>
					<textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html($item->description); // textarea_escaped ?></textarea>
					<span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.', 'fleur'); ?></span>
				</label>
			</p>
			<?php
			...
			<p class="field-custom description description-thin description-thin-custom">
				<label for="edit-menu-item-anchor-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Anchor', 'fleur'); ?><br/>
			...
			<p class="field-custom description description-wide">
				...
				<label for="edit-menu-item-nolink-<?php echo esc_attr($item_id); ?>">
					...
					<?php esc_html_e("Don't link", 'fleur'); ?>
				</label>
			</p>
			...
			<p class="field-custom description description-thin description-thin-custom">
				<label for="edit-menu-item-wide-position-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Wide menu position', 'fleur'); ?><br/>
			...
			<p class="field-custom description description-wide">
				<label for="edit-menu-item-background-image-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Background image for Dropdown', 'fleur'); ?><br/>
			...
			<?php
			$iconCollections = fleur_mikado_icon_collections()->getIconCollectionsEmpty();
			...
			if(is_array($iconCollections) && count($iconCollections)) {
				...
				<p class="field-custom description description-thin description-thin-custom">
					<label for="edit-menu-item-icon-pack-<?php echo esc_attr($item_id); ?>">
						<?php esc_html_e('Icon Pack', 'fleur'); ?><br/>
						...
						<br/><?php esc_html_e('Only with "default" & "wide with icons" menu types', 'fleur'); ?>
					</label>
				</p>

				<?php
				$icon_data_attr = 'menu_item_icon_'.$item_id;
				$collection_obj = fleur_mikado_icon_collections()->getIconCollection($item->icon_pack);

				?>
				<p class="field-custom description description-thin description-thin-custom mkd-icon-select-holder">
					<label for="edit-menu-item-icon-<?php echo esc_attr($item_id); ?>">
						<?php esc_html_e('Icon', 'fleur'); ?><br/>
			...
			<p class="field-custom description description-thin description-thin-custom" style="clear:both;">
				<label for="edit-menu-item-featured-icon-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Featured icon', 'fleur'); ?><br/>
					<select class="widefat" id="edit-menu-item-featured-icon<?php echo esc_attr($item_id); ?>" data-item-option data-name="menu_item_featured_icon_<?php echo esc_attr($item_id); ?>">
						...
						<option value="icon_star" <?php if($item->featured_icon == "icon_star") {
							echo 'selected="selected"';
						} ?>><?php esc_html_e('Star', 'fleur'); ?></option>
						<option value="icon_heart " <?php if($item->featured_icon == "icon_heart ") {
							echo 'selected="selected"';
						} ?>><?php esc_html_e('Heart', 'fleur'); ?></option>
						<option value="arrow_right-up" <?php if($item->featured_icon == "arrow_right-up") {
							echo 'selected="selected"';
						} ?>><?php esc_html_e('Arrow', 'fleur'); ?></option>
					</select>
				</label>
			</p>
			...
			<p class="field-custom description description-wide">
				<label for="edit-menu-item-sidebar-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Custom widget area', 'fleur'); ?><br/>
					...
					<br/><?php esc_html_e('Only with "wide & wide with icons" menu type', 'fleur'); ?>
				</label>
			</p>
			<?php
			...
			<div class="menu-item-actions description-wide submitbox">
				<?php if('custom' != $item->type && $original_title !== false) : ?>
					<p class="link-to-original">
						<?php printf(esc_html__('Original: %s', 'fleur'), '<a href="'.esc_attr($item->url).'">'.esc_html($original_title).'</a>'); ?>
					</p>
				<?php endif; ?>
				...
				)); ?>"><?php esc_html_e('Remove', 'fleur'); ?></a> <span class="meta-sep"> | </span>
				<a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url(add_query_arg(array(
					'edit-menu-item' => $item_id,
					'cancel'         => time()
				), remove_query_arg($removed_args, admin_url('nav-menus.php'))));
				?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel', 'fleur'); ?></a>
			</div>
