 ...
 */
if(!class_exists('FleurMikadoTopNavigationWalker')) {
	class FleurMikadoTopNavigationWalker extends Walker_Nav_Menu {
		...
		function start_lvl(&$output, $depth = 0, $args = array()) {
			...
			if($depth == 0) {
				$out_div = '<div class="second '.$background_image_class.'" '.fleur_mikado_get_inline_style($menu_image_background).'><div class="inner">';
			} else {
				$out_div = '';
			}
		...
		function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
			...
			if($item->hide == '') {
				...
				if(!empty($icon) && fleur_mikado_get_meta_field_intersect('header_type', fleur_mikado_get_page_id()) !== 'header-vertical') {
				...
				if(!empty($item->description) && fleur_mikado_get_meta_field_intersect('header_type', fleur_mikado_get_page_id()) !== 'header-vertical') {
