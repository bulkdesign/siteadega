...
class FleurMikadoFramework {
	...
	private function __construct() {
		$this->mkdOptions   = FleurMikadoOptions::get_instance();
		$this->mkdMetaBoxes = FleurMikadoMetaBoxes::get_instance();
	}
	...
	public function setSkin(FleurMikadoSkinAbstract $skinObject) {
		$this->skin = $skinObject;
	}
}

/**
 * Class FleurMikadoSkinManager
 *
...
/**
 * Class FleurMikadoSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class FleurMikadoSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	/**
	 * Loads template part with params. Uses fleur_mikado_find_template_path function which is child theme friendly
	 *
	...
	public function loadTemplatePart($template, $params = array()) {
		...
		if($template !== '') {
			include(fleur_mikado_find_template_path('framework/admin/skins/'.$this->skinName.'/templates/'.$template.'.php'));
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.mkdSCIcon = '<?php echo fleur_mikado_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
...
class FleurMikadoOptions {
	...
	public function getOptionValue($key) {
		...
		if(array_key_exists($key, $fleur_options)) {
			return $fleur_options[$key];
...
/*
   Class: FleurMikadoAdminPage
   A class that initializes Mikado Admin Page
*/

class FleurMikadoAdminPage implements iFleurMikadoLayoutNode {
	...
	function __construct($slug = "", $title = "", $icon = "") {
		$this->layout  = array();
		$this->factory = new FleurMikadoFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild(iFleurMikadoRender $child) {
		$child->render($this->factory);
	}
}

/*
   Class: FleurMikadoMetaBoxes
...
/*
   Class: FleurMikadoMetaBox
   A class that initializes Mikado Meta Box
*/

class FleurMikadoMetaBox implements iFleurMikadoLayoutNode {
	...
	public function setTitle($label) {
		...
		return $fleur_Framework->getSkin()->getSkinLabel().' '.$label;
	}
}

global $fleur_Framework;

$fleur_Framework = FleurMikadoFramework::get_instance();
$mkdSkinManager                 = new FleurMikadoSkinManager();
$fleur_Framework->setSkin($mkdSkinManager->getSkin());
