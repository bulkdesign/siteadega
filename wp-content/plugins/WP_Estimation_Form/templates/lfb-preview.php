...
wp_register_style($lfb->_token . '-bootstrap-timepicker', esc_url($lfb->assets_url) . 'css/timepicker.min.css', array(), $lfb->_version);
...
//wp_enqueue_style($lfb->_token . '-bootstrap-timepicker');
...
wp_register_script($lfb->_token . '-bootstrap-timepicker', esc_url($lfb->assets_url) . 'js/bootstrap-timepicker.min.js', array($lfb->_token . '-bootstrap'), $lfb->_version);
wp_enqueue_script($lfb->_token . '-bootstrap-timepicker');
...
if ($form) {
    ...
    if($form->gmap_key != ""){
     $chkMap = false;
        ...
        if($chkMap){
            wp_register_script($lfb->_token . '-gmap', 'http://maps.googleapis.com/maps/api/js?key='.$form->gmap_key.$libPlace,array());
