...
class FleurMikadoLinearIcons implements iFleurMikadoIconCollection {
	...
	public function __construct($title = "", $param = "") {
		...
		$this->title    = $title;
		$this->param    = $param;
		$this->styleUrl = MIKADO_ASSETS_ROOT."/css/linear-icons/style.css";
		$this->setIconsArray();
	}
	...
	public function render($icon, $params = array()) {
		...
		$html .= '<i class="mkd-icon-linear-icon lnr '.$icon.' '.$iconClass.'" '.$iconAttributesString.'></i>';
